...
class VersionComplianceController extends BaseController
{
    ...
    public function index(Request $request, $complianceVersionId)
    {
        ...
        $tree = getClauseTree($version->standard_id, $request->clause_id_filter);

        $data['no_pagination'] = true;

        return view($this->route . "/index")
            ->with(['data' => $data, 'items' => $tree, 'standardId' => $version->standard_id, 'request' => $request, 'version' => $complianceVersionId, 'route' => $this->route, 'heading' => "Compliance for " . ($version->standard->name ?? '') . " - Version : " . $version->name ?? '', 'version_id' => $version->id]);
    }
