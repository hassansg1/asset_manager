              ...
              <div class="col-lg-6">
                  <div class="mb-3">
                      ...
                      <select class="form-control select2" name="user_type" id="user_type" required>
                          ...
                          <option value="asset" {{ isset($item) && $item->parent == "asset"  ? 'selected' : ''}}>Asset User ID</option>
                          <option value="system" {{ isset($item) && $item->parent == "system"  ? 'selected' : ''}}>System User ID</option>
                      </select>
                  </div>
              </div>
                  ...
                  <div class="mb-3">
                      ...
                      <select class="form-control select2" id="system_id" name="system_id">
                          ...
                          @foreach($system as $value)
                              <option value="{{$value->id}}" {{ isset($item) && $item->parent_id == $value->id  ? 'selected' : ''}}>{{$value->name}}</option>
                          @endforeach
                      </select>
                  </div>
              </div>
                  ...
                  <div class="mb-3">
                      ...
                      <select class="form-control select2" id="asset_id" name="asset_id">
                          ...
                          @foreach($assets as $value)
                              <option value="{{$value->id}}" {{ isset($item) && $item->parent_id == $value->id  ? 'selected' : ''}}>{{$value->rec_id}}</option>
                          @endforeach
                      </select>
                  </div>
              </div>
           ...
           <div class="col-lg-4">
            <div class="mb-3">
              <label for="{{ isset($item) ? $item->id:'' }}right"
                ...
                <select class="form-control select2" id="right_id" name="right_id" required>
                  ...
                  <option value="{{$value->id}}" {{ isset($item) && $item->user_rights_id->right_id == $value->id  ? 'selected' : ''}}>{{$value->name}}</option>
                  @endforeach
                </select>
              </div>
            </div>
            <div class="col-lg-4">
                <div class="mb-3">
                    ...
                    <select class="form-control select2" id="otcm_user_id" name="otcm_user_id">
                        <option value="">-Select User-</option>
                        @foreach($otcm_users as $value)
                            <option value="{{$value->id}}" {{ isset($item) && $item->otcm_user_id == $value->id  ? 'selected' : ''}}>{{$value->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
